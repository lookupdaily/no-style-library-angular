...
describe('InputComponent', () => {
  ...
  it('should display any reactive form errors if matching validation message is given', () => {
    ...
    expect(fixture.debugElement.nativeElement.innerHTML).not.toContain(
      'Value should be less than 5 characters'
    );
  ...
  describe('When select type is specified', () => {
    beforeEach(() => {
      ...
      inputComponent.ngOnChanges({
        type: createSimpleChange(inputComponent.type),
      });
      fixture.detectChanges();
    });
  ...
  describe('When checkbox type is specified with multiple options', () => {
    ...
    it('renders checkbox group component', () => {
      const checkboxGroupComponent = fixture.debugElement.query(
    ...
    it('should listen for change event and emit change', () => {
      ...
      checkboxGroupComponent.dispatchEvent(event);
      expect(inputComponent.change.emit).toHaveBeenCalled();
    });
  });
